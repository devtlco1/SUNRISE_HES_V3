import {
  postTcpListenerObisJobCancelToPythonSidecar,
...
export async function POST(
  ...
  try {
    const out = await postTcpListenerObisJobCancelToPythonSidecar(jobId)
    ...
    if (e instanceof PythonSidecarHttpError) {
      if (e.status === 404) {
        try {
          ...
          return NextResponse.json(
            { ok: false, error: "CANCEL_REJECTED", message: jobId },
            { status: 404 }
          )
        }
      }
