...
def _relay_audit(
    ...
    *,
    ...
    payload = {
        "phase": phase,
        "operation": operation,
        "target_meter_serial": (meter_id or "").strip(),
    }
    if extra:
        payload.update(extra)
    log.info("tcp_listener_relay_audit", extra=payload)
...
def _run_tcp_listener_relay(
    ...
    *,
    ...
    exclusive_relay_state_change: bool = False,
) -> RuntimeResponseEnvelope:
    ...
    if exclusive_relay_state_change:
        ctl.relay_state_change_begin()
    try:
            ...
            with ctl.session_context():
                try:
                    ...
                    _relay_audit(
                        ...
                        extra={
                            "exclusive_relay_state_change": exclusive_relay_state_change,
                    ...
                    _relay_audit(
                        ...
                        extra={
                            "remote": remote,
                            "hold_id": hold_id,
                            "identity_just_verified": acq.identity_just_verified,
                            "session_bound_before_pop": not acq.identity_just_verified,
                        },
                    )
                    ...
                    try:
                        if require_preflight_identity and not acq.identity_just_verified:
                            ...
                            _relay_audit(
                                ...
                                operation=operation,
                                meter_id=request.meterId,
                                extra={"remote": remote, "hold_id": hold_id},
                            )
                        elif require_preflight_identity:
                            _relay_audit(
                                "preflight_skipped_identity_just_verified",
                                operation=operation,
                                meter_id=request.meterId,
                                extra={"remote": remote, "hold_id": hold_id},
                            )
                        ...
                        _relay_audit(
                            ...
                            extra={
                                ...
                                "detail_code": getattr(diags, "detailCode", None) if diags else None,
                                "verified_on_wire": getattr(diags, "verifiedOnWire", None)
                                if diags
                                else None,
                                "association_attempted": getattr(
                                    diags, "associationAttempted", None
                                )
                                if diags
                                else None,
                                "payload_relay_state": getattr(envelope.payload, "relayState", None)
        ...
        if exclusive_relay_state_change:
            ctl.relay_state_change_end()
        fin = datetime.now(timezone.utc)
        diags = envelope.diagnostics if envelope is not None else None
        _relay_audit(
            "action_exit",
            operation=operation,
            meter_id=request.meterId,
            extra={
                ...
                "hold_id": hold_id,
                "teardown": teardown,
                "duration_ms": max(1, int((fin - started).total_seconds() * 1000)),
                "final_envelope_ok": envelope.ok if envelope is not None else None,
                "final_detail_code": getattr(diags, "detailCode", None) if diags else None,
                "final_verified_on_wire": getattr(diags, "verifiedOnWire", None)
                if diags
                else None,
                "exclusive_relay_state_change": exclusive_relay_state_change,
            },
...
def execute_tcp_listener_relay_disconnect(request: ReadIdentityRequest) -> RuntimeResponseEnvelope:
    return _run_tcp_listener_relay(
        ...
        exclusive_relay_state_change=True,
    )
