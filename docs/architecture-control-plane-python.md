...
3. **Internal** routes (server-only, when `RUNTIME_PYTHON_SIDECAR_URL` is set):
   ...
   - `POST /api/internal/python-runtime/tcp-listener/read-basic-registers` → Python `POST /v1/runtime/tcp-listener/read-basic-registers` (same staged-socket pattern)
